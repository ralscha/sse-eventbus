...
public class InMemoryReplayStore implements ReplayStore {
	 ...
	 * best-effort fallback so the client receives as much history as possible after a long
	 * absence. Callers should be aware that this may result in some duplicate events being
	 * delivered if the client re-connects after the event has already been received but
	 * before its ID was purged from the store.
	 */
	@Override
	public List<ReplayEvent> getEventsSince(String clientId, String lastEventId) {
		ConcurrentLinkedDeque<ReplayEvent> events = this.replayEvents.get(clientId);
		if (events == null || events.isEmpty()) {
			return List.of();
		}

		List<ReplayEvent> result = new ArrayList<>();
		String requestedLastEventId = lastEventId;
	...
	public void purgeExpired(long expirationTimestamp) {
		this.replayEvents.forEach((clientId, events) -> {
			while (true) {
				ReplayEvent first = events.peekFirst();
